...
describe("PUT /admin/blog-tags/:id", () => {
  ...
  it("updates a blog tag record", async () => {
    updateMock.mockResolvedValue({ id: validId, name: "Blog Tag Diperbarui" } as never);

    const response = await request(app)
      .put(`/admin/blog-tags/${validId}`).set("Authorization", "Bearer admin-token")
      .send({ name: "Blog Tag Diperbarui" });

    expect(response.status).toBe(200);
    expect(response.body).toHaveProperty("data");
    expect(response.body.data).toMatchObject({ id: validId, name: "Blog Tag Diperbarui" });
    expect(typeof response.body.data.name).toBe("string");
  });

  it("returns 403 when a non-admin user updates the resource", async () => {
    const response = await request(app)
      .put(`/admin/blog-tags/${validId}`).set("Authorization", "Bearer user-token")
      .send({ name: "Blog Tag Diperbarui" });
