...
describe("DELETE /admin/blogs/:id", () => {
  ...
  it("deletes the admin blog resource", async () => {
    ...
    const response = await request(app)
      .delete(`/admin/blogs/${validId}`).set("Authorization", "Bearer admin-token");
  ...
  it("returns 403 for non-admin users", async () => {
    const response = await request(app)
      .delete(`/admin/blogs/${validId}`).set("Authorization", "Bearer user-token");
  ...
  it("returns 404 when the admin blog cannot be found", async () => {
    deleteMock.mockRejectedValue(new ResponseError(404, "Admin Blog tidak ditemukan"));
