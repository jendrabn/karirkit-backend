...
describe("DELETE /admin/blog-categories/mass-delete", () => {
  ...
  it("deletes multiple blog category records", async () => {
    ...
    const response = await request(app)
      .delete("/admin/blog-categories/mass-delete").set("Authorization", "Bearer admin-token")
  ...
  it("returns 403 for non-admin users", async () => {
    const response = await request(app)
      .delete("/admin/blog-categories/mass-delete").set("Authorization", "Bearer user-token")
